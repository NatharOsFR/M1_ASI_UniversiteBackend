...
public class CreateParcoursUseCase(IRepositoryFactory parcoursRepository)
{
    ...
    public async Task<Parcours> ExecuteAsync(Parcours parcours)
    {
        ...
        Parcours pa = await parcoursRepository.ParcoursRepository().CreateAsync(parcours);
        parcoursRepository.ParcoursRepository().SaveChangesAsync().Wait();
        return pa;
    }
    private async Task CheckBusinessRules(Parcours parcours)
    {
        ...
        ArgumentNullException.ThrowIfNull(parcoursRepository.ParcoursRepository());
